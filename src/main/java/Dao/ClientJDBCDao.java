...
// Забираем из результирующего набора получившиеся записи. Колонки это порядковые номера нашего запроса.
    ...
    public void updateAge(int age, int id) {
        ...
        try {
            preparedStatement = connection.prepareStatement("UPDATE clients SET age = ? WHERE id_clients = ?");
    ...
    private Connection getConnection() {
        ...
        try {
            connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/carsshop?useSSL=false", "root", "Ruslan2703");
            return connection;
        } catch (SQLException e) {
            e.printStackTrace();
        }
